...
@testable import OtlpGRPCExporter
import Tracing
import XCTest
...
final class TypeConversionTests: XCTestCase {
    ...
    func test_convertInstrumentationLibrarySpans() {
        ...
        XCTAssertEqual(
            Opentelemetry_Proto_Trace_V1_ScopeSpans(spans: [span]),
            .with {
                $0.spans = [.init(span)]
            }
        )
    ...
    func test_convertSpanAttribute_string() {
        XCTAssertEqual(Opentelemetry_Proto_Common_V1_AnyValue(SpanAttribute.string("test")), .with { $0.stringValue = "test" })
    }
    ...
    func test_convertSpanAttribute_stringConvertible() {
        XCTAssertEqual(Opentelemetry_Proto_Common_V1_AnyValue(SpanAttribute.stringConvertible(42)), .with { $0.stringValue = "42" })
    }
